...
# 简单但有效的等待函数
def wait_for_ros_ready(max_wait_time=45):
    """等待ROS系统就绪，带有进度显示"""
    print(f"{BLUE}[WAIT]{RESET} 等待ROS系统启动（最多{max_wait_time}秒）...")

    # 基础等待时间
    base_wait = 8
    print(f"{BLUE}[WAIT]{RESET} 基础等待 {base_wait} 秒...")
    for i in range(base_wait):
        print(f"⏱️  {i+1}/{base_wait}", end="\r")
        time.sleep(1)
    print()

    # 检查关键节点
    print(f"{BLUE}[CHECK]{RESET} 检查关键节点...")
    remaining_time = max_wait_time - base_wait

    for attempt in range(remaining_time):
        try:
            # 检查是否有足够的ROS节点运行
            ...
            if result.returncode == 0:
                ...
                node_count = len([n for n in nodes if n.strip()])

                # 检查关键节点存在
                has_controller = any('controller' in node for node in nodes)
                has_mpc = any('mpc' in node for node in nodes)

                if node_count >= 3 and (has_controller or has_mpc):
                    print(f"{GREEN}✓{RESET} 检测到 {node_count} 个节点，系统似乎就绪")
                    ...
                    try:
                        ...
                        if "/joint_cmd" in topic_result.stdout:
                            print(f"{GREEN}✓{RESET} 关键topic已发布，系统就绪")
                            # 最后的安全等待
                            print(f"{BLUE}[FINAL]{RESET} 最后安全等待5秒...")
                            time.sleep(5)
                            return True
        ...
        print(f"⏱️  等待中... {attempt+1}/{remaining_time}", end="\r")
        time.sleep(1)

    print()
    print(f"{YELLOW}⚠{RESET} 等待超时，但继续执行（可能系统已部分就绪）")
    return False
...
def run_task(task_id: int, headless: bool):
    ...
    while True:
        ...
        try:
            # ⭐ 关键改进：智能等待而不是固定2秒
            wait_for_ros_ready(max_wait_time=45)
